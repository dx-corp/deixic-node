...
export const INTERNAL_MAESTRO_PRODUCT_HTTP_STATUS_HEADER = "x-maestro-product-http-status";

/** Stable error categories for product code. */
export type MaestroProductErrorKind =
  | "authentication"
  | "authorization"
  | "validation"
  | "conflict"
...
/**
 * An error returned by the Maestro Product Kit facade. The original Connect
 ...
 */
export class MaestroProductError extends Error {
  readonly kind: MaestroProductErrorKind;
...
export function asMaestroProductError(error: unknown): MaestroProductError {
  if (error instanceof MaestroProductError) return error;
  if (error instanceof ConnectError) return fromConnectError(error);

  return new MaestroProductError({
...
function fromConnectError(error: ConnectError): MaestroProductError {
...
function preservedHttpStatus(metadata: Headers): number | undefined {
  const value = metadata.get(INTERNAL_MAESTRO_PRODUCT_HTTP_STATUS_HEADER);
  if (!value || !/^[45]\d{2}$/.test(value)) return undefined;
  return Number(value);
}

function errorKindFor(code: Code): MaestroProductErrorKind {
  switch (code) {
