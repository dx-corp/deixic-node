...
import type { MaestroProductClient } from "./client.js";
import { MaestroProductError, asMaestroProductError } from "./errors.js";
...
export interface MaestroObserveAcceptedTurnInput {
   ...
   */
  acceptance: MaestroAcceptedTurnAcceptance
  ...
  watch?: MaestroAcceptedTurnWatchOptions;
...
interface MaestroAcceptedTurnResultBase {
...
export type MaestroAcceptedTurnUnfinishedReason =
  | "backfill_limit"
  | "backfill_error"
  | "watch_not_requested"
  | "watch_eof"
 ...
 */
export type MaestroAcceptedTurnResult =
  | (MaestroAcceptedTurnResultBase & { status: "responded" })
  | (MaestroAcceptedTurnResultBase & { status: "completed" })
  | (MaestroAcceptedTurnResultBase & { status: "failed" })
  | (MaestroAcceptedTurnResultBase & { status: "interrupted" })
  | (MaestroAcceptedTurnResultBase & { status: "waiting" })
  | (MaestroAcceptedTurnResultBase & {
    status: "unfinished";
    reason: MaestroAcceptedTurnUnfinishedReason;
 ...
 */
export async function observeMaestroAcceptedTurn(
  client: MaestroProductClient,
  input: MaestroObserveAcceptedTurnInput,
): Promise<MaestroAcceptedTurnResult> {
  ...
  const base = (): MaestroAcceptedTurnResultBase => ({
  ...
  const unfinished = (
    reason: MaestroAcceptedTurnUnfinishedReason,
    error?: MaestroProductError,
  ): MaestroAcceptedTurnResult => ({
    ...base(),
    status: "unfinished",
    reason,
    ...(error ? { error } : {}),
  });
  const cancelled = (): MaestroAcceptedTurnResult => unfinished("cancelled");
  ...
  const processPage = async (page: ObservationPage): Promise<MaestroAcceptedTurnResult | undefined> => {
    ...
    for (const record of matchingEvents) {
      ...
      const eventId = record.event.eventId.trim();
  ...
  for (let pageNumber = 0; pageNumber < maxBackfillPages && hasMore; pageNumber += 1) {
    ...
    try {
      ...
      return unfinished("backfill_error", asMaestroProductError(error));
    }
  }
  ...
  try {
    ...
    if (signal?.aborted) return cancelled();
    rethrowObservationError(error);
    return unfinished("watch_error", asMaestroProductError(error));
  }
...
type ObservationPage = {
  ...
  threadExecution?: OperatingThreadExecution;
...
function protocolError(message: string): ObservationProtocolError {
  return new ObservationProtocolError(new MaestroProductError({
    message,
    kind: "protocol",
    status: 502,
  }));
...
class ObservationProtocolError extends Error {
  constructor(readonly productError: MaestroProductError) {
