...
export const MAESTRO_PRODUCT_APP_CONTEXT_HEADER = "X-EvalOps-App-Context";

/** Matches the existing Operating Chat header budget. */
export const MAESTRO_PRODUCT_APP_CONTEXT_MAX_HEADER_CHARS = 6000;
 ...
 * record.
 */
export function encodeMaestroProductAppContextHeader(value: unknown): string | undefined {
  const normalized = normalizeForHeader(value);
  if (normalized === undefined) return undefined;

  const encoded = encodedNormalizedContext(normalized);
  if (encoded.length <= MAESTRO_PRODUCT_APP_CONTEXT_MAX_HEADER_CHARS) return encoded;
 ...
 */
export function encodedMaestroProductAppContextHeaderLength(value: unknown): number {
  const normalized = normalizeForHeader(value);
